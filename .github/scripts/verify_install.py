...
Naming note: the repository is `freecad-gemstone`, but the Python namespace
package, the workbench class and every command are `lapidary`/`Lapidary`.
Those names are what FreeCAD actually loads, so they are what this script
checks. Do not "fix" these to gemstone without renaming the package itself.
"""
import os
import py_compile
import sys
